...
<head>
    <meta charset="UTF-8" />
    <title>Testimoni Disetujui - Admin UIN Raden Mas Said</title>
    ...
    <div class="flex flex-1 flex-col md:flex-row">
       ...
       {{-- Sidebar --}}
        ...
        <main class="flex-1 p-4 sm:p-6 lg:p-8 flex flex-col">
            ...
            {{-- Header/Title Section (Approved Page) --}}
            <header class="mb-8 p-4 bg-white rounded-xl shadow-md flex items-center justify-between">
                <div>
                    <h1 class="text-3xl lg:text-4xl font-extrabold text-green-700 tracking-tight font-['Poppins']">
                        Testimoni Disetujui
                    </h1>
                    <p class="text-gray-600 text-lg mt-1">Daftar testimoni yang sudah disetujui dan ditampilkan di website.</p>
                </div>
            </header>
            ...
            <div class="container mx-auto bg-white rounded-2xl shadow-2xl border border-gray-200 flex-1">
                ...
                {{-- Stats --}}
                <div class="p-6 pb-4 border-b border-gray-200 bg-green-50 rounded-t-2xl flex justify-between items-center">
                    {{-- Menggunakan variabel $testimonialsApproved dari Controller --}}
                    <h3 class="text-lg font-bold text-green-700">Testimoni Disetujui ({{ $testimonialsApproved->total() ?? 0 }})</h3>
                </div>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white p-5 rounded-xl shadow-lg border border-green-200">
                            ...
                            <p class="text-2xl font-bold text-green-700 mb-3 flex items-center gap-2">
                                <i data-lucide="check-square" class="w-6 h-6"></i> Testimoni Disetujui
                            </p>

                            <blockquote class="text-lg italic text-gray-700 border-l-4 border-green-400 pl-4 mb-4">
                            ...
                            <div class="mt-5 pt-4 border-t border-gray-100 flex justify-end items-center">
                                ...
                                {{-- Tombol Tarik Publikasi (Menggunakan aksi 'reject' untuk memindahkan ke status 'rejected') --}}
                                <form action="{{ route('admin.testimonials.reject', $alumni->user_id) }}" method="POST" onsubmit="return confirm('Tarik publikasi testimoni ini dan pindahkan ke daftar Ditolak?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="flex items-center gap-2 bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-2 px-4 rounded-full shadow-md text-sm transition transform hover:-translate-y-0.5">
                                        <i data-lucide="x-circle" class="w-4 h-4"></i> Tarik Publikasi
                                    </button>
                                </form>
                        ...
                        <div class="md:col-span-2 text-center py-10 bg-gray-50 rounded-xl">
                            <i data-lucide="check-check" class="w-12 h-12 text-green-400 mx-auto mb-3"></i>
                            <p class="text-lg text-gray-500 font-semibold">Tidak ada testimoni yang dipublikasikan saat ini.</p>
                        </div>
                        @endforelse
                    </div>

                    {{-- Pagination --}}
                    @if ($testimonialsApproved->lastPage() > 1)
                    <div class="mt-8">
