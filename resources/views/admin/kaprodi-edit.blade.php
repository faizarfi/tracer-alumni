...
@section('header')
    ...
    <header class="mb-6 p-4 bg-white rounded-xl shadow-md flex flex-col md:flex-row items-center justify-between animate-fade-in gap-4">
        <div class="flex items-center w-full md:w-auto">
            ...
            <button id="sidebarToggle" class="mr-3 text-emerald-700 md:hidden p-2 rounded-lg hover:bg-emerald-100 transition duration-150" aria-label="Toggle Menu">
                <i data-lucide="menu" class="w-6 h-6"></i>
            </button>
            <div>
                <h1 class="text-xl lg:text-2xl font-extrabold text-emerald-800 tracking-tight font-['Poppins']">
                    Edit Data Kaprodi
                </h1>
                <p class="text-emerald-600 text-xs md:text-sm mt-0.5">Memperbarui informasi: <span class="font-bold">{{ $kaprodi->name ?? 'N/A' }}</span></p>
            </div>
        </div>
        <div class="flex flex-col items-center md:items-end w-full md:w-auto bg-emerald-50 md:bg-transparent p-2 md:p-0 rounded-lg">
...
@section('content')
    ...
    <div class="container mx-auto max-w-4xl bg-white rounded-2xl shadow-xl border border-gray-200 p-6 md:p-8 animate-fade-in">
        ...
        <form action="{{ route('admin.kaprodi.update', $kaprodi->id) }}" method="POST" class="space-y-8">
            ...
            {{-- Section: Data Dasar --}}
            <div class="space-y-5 p-5 md:p-6 border border-emerald-100 rounded-2xl bg-emerald-50/50">
                <h3 class="text-md font-bold text-emerald-800 flex items-center gap-2 mb-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $kaprodi->name) }}" required
                            class="w-full px-4 py-2.5 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition text-sm"
            ...
            {{-- Tombol Aksi Responsif --}}
                ...
                <button type="submit"
                    class="w-full md:w-auto inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-8 rounded-xl shadow-lg shadow-emerald-100 transition-all duration-300 transform hover:scale-[1.03] active:scale-95">
